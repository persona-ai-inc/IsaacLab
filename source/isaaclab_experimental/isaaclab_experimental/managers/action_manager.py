...
class ActionTerm(ManagerTermBase):
    ...
    * Applying actions: This operation is performed once per **simulation step** and is
    ...
    def process_actions(self, actions: wp.array, action_offset: int = 0):
        ...
        Args:
            actions: The full action buffer of shape (num_envs, total_action_dim).
            action_offset: Column offset into the action buffer for this term.
        """
        raise NotImplementedError
...
class ActionManager(ManagerBase):
    ...
    * apply actions: This operation typically sets the processed actions into the assets in the
    ...
    def __init__(self, cfg: object, env: ManagerBasedEnv):
        ...
        super().__init__(cfg, env)
        # create buffers to store actions (Warp)
        self._action = wp.zeros((self.num_envs, self.total_action_dim), dtype=wp.float32, device=self.device)
        self._prev_action = wp.zeros((self.num_envs, self.total_action_dim), dtype=wp.float32, device=self.device)
    ...
    def get_active_iterable_terms(self, env_idx: int) -> Sequence[tuple[str, Sequence[float]]]:
        ...
        terms = []
        idx = 0
        # Copy to host for debug/inspection (not on hot path).
        for name, term in self._terms.items():
            term_actions = self._action_torch[env_idx, idx : idx + term.action_dim]
