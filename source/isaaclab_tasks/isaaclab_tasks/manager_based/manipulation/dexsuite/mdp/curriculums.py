...
def initial_final_interpolate_fn(env: ManagerBasedRLEnv, env_id, data, initial_value, final_value, difficulty_term_str):
    """Interpolate between initial and final values scaled by the current difficulty fraction.
...
class DifficultyScheduler(ManagerTermBase):
    ...
    Promotes difficulty when the ``success`` reward term's sticky
    :attr:`succeeded` flag is ``True`` for an environment at episode end,
    meaning the agent achieved the success condition at least once during the
    episode.  Demotes (unless ``promotion_only`` is set) otherwise.

    The normalized average difficulty across environments is exposed as
    :attr:`difficulty_frac` for use in curriculum interpolation.
    """
    ...
    def __call__(
        ...
        succeeded = env.reward_manager.get_term_cfg("success").func.succeeded[env_ids]
